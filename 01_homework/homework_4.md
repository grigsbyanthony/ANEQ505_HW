...
1. Describe one way to get data from your qiime2 outputs into a format that can be used for R. **Export the artifact to TSV format w/ gunzip or a more particular command and then read into R with read_tsv().**

2. Which body site appeared most distinct in the taxa bar plot, meaning it was not similar to at least one of the other body sites? Explain why that site looks different. **Fecal seems like its generally the most variable one, which is probably driven by the abundance of Cryptobacteroides and Treponema_D (things you’d expect in an anaeobic gut community).**

3. When generating the filtered table for ANCOM-BC2, what value did you choose for `--p-min-frequency`? Which core metrics parameter should this match, and why do these values need to be the same? (Report your core metrics value here: **Matched the rarefaction depth used in core-metrics/core-metrics-phylogenetic with the idea that our ANCOM-BC2 and our diversity analyses will use the same minimum per-sample sequencing depth. They oughta match so we’re comparing the same subset of samples across alpha/beta diversity and differential abundance analyses.**

4. Why do we filter out samples with low frequency and low abundance ASVs? **To reduce noise and compositional artifacts and ultimately improve the power behind our statistics once we run them. That way we can at least somewhat avoid spurious results in our differentially abundant calls.**

5. What was the most enriched genus in skin compared to fecal, and what was the most depleted genus in skin compared to fecal (make sure adjusted p is set to less than 0.05)? **Atopostipes in Skin vs Streptococcus in fecal just barely.**
...
### **Questions:**
1. Why might removing controls be important before downstream analysis? **Controls contain contaminants that aren’t really reflective of any real biological signal, so including them would train classifiers on artifacts and hurt their accuracy.**
2. what 2 features that are high in fecal samples? **Cryptobacteroides and Alistipes_A are pretty high importance in fecal.**
3. what are 2 features that are low in nasal? **Lactiplantibacillus, Treponema_D, and Weissella are all low importance in nasal.**
4. what is the accuracy of your model, and if the accuracy of the classifier is high, what does that suggest about the microbial compositions of each site? **~87.5%, so it seems body sites harbor sufficiently distinct microbial communities that genus-level composition can predict sample site origin with decent reliability.**